...
const VisaPerfectionCard = ({ data }) => {
    return (
        <div className='w-full mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4'>
            {data?.map((item, i) => (
                <div
                    key={i}
                    className='w-full flex flex-col items-center gap-5 py-14 p-5 bg-[#10111A] rounded-lg border-2 border-[#D0A148]'
                >
                    <div>{item?.icon}</div>
...
export default VisaPerfectionCard;